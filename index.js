  ...
  while (true) {
    ...
    if (choice.start == "View all departments") {
      ...
      console.table(data);
        ...
        const mngr_choices = mngr_list.map(({ id, first_name, last_name }) => ({ 'name': first_name + " " + last_name, 'value': id }));
        const role_list = await query.getAllRoles();
        const role_choices = role_list.map(({ id, title }) => ({ 'name': title, 'value': id }));
        ...
        await query.addEmployee(employee.first_name, employee.last_name, employee.title, employee.manager);
    } else if (choice.start == "Update an employee role") {
        const employee_list = await query.getAllEmployees();
        const employee_choices = employee_list.map(({ id, first_name, last_name }) => ({ 'name': first_name + " " + last_name, 'value': id }));
        const role_list = await query.getAllRoles();
        const role_choices = role_list.map(({ id, title }) => ({ 'name': title, 'value': id }));
